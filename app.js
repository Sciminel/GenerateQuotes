...
let citations = [
  ...
  const txtQuotes = document.querySelector('.quotes');
  const authorQuotes = document.querySelector('.author');
  const valeurInput = document.querySelector('#name');
  ...
  btn.addEventListener('click', (e) => {
    ...
    for(let i = 0; i < citations.length; i++){
        ...
        if(citations[i][1].includes(valeurInput.value)){
            txtQuotes.innerText = `"${citations[i][0]}"`;
            authorQuotes.innerText = `Auteur : ${citations[i][1]}`
        }else{
            txtQuotes.innerText = "Je ne trouve pas de citations qui contient cette valeur";
        }
        console.log(txtQuotes)
    }
  })
  ...
  btnAuto.addEventListener('click', (e) => {
      ...
      let index = Math.trunc(Math.random()*21);
      console.log(index)
      txtQuotes.innerText = `"${citations[index][0]}"`;
      authorQuotes.innerText = `Auteur : ${citations[index][1]}`
